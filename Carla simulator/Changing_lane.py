...
class ManeuverAgent():
    def __init__(self, world, bp,actor_list, vel_ref=VEHICLE_VEL, max_throt=1, max_brake=0.1, max_steer=1):
        self.world = world
    ...
    def create_route(self, start_waypoint, end_waypoint):
        """
        ...
        if self.grp is None:
            dao = GlobalRoutePlannerDAO(self.vehicle.get_world().get_map(), self.hop_resolution)
    ...
    def set_destination(self, location):
        ...
        start_waypoint = self._map.get_waypoint(self.vehicle.get_location())
...
def spawn_traffic(world, spawn_points, blueprint_library, actor_list):
...
def check_for_lane(world,waypoint):
    ...
    if world.get_map() == 'Carla/Maps/Town04':
...
def move_to_hospital(agent, vehicle, hospitals_locations, Map):
    state = 2
    loc = 5
    while True:
        if state == 1 and loc > 20:
            near_location_ind,loc = get_nearest_hospital(vehicle, hospitals_locations, Map)
            agent.set_destination(vehicle.get_location(),hospitals_locations[near_location_ind].location)

        if state == 2:
            near_location,loc = get_nearest_hospital(vehicle, hospitals_locations, Map)
            agent.set_destination(hospitals_locations[near_location].location)
            state = 0
...
def main():
    ...
    try:
        ...
        agent = BasicAgent(Maneuver_Agent.vehicle)
            
        hospitals_locations = spawn_hosbitals(blueprint_library, world, actor_list)
        spawn_traffic(world, spawn_points, blueprint_library,actor_list)
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_q:
                        ...
                        if len(world.get_map().get_waypoint(Maneuver_Agent.vehicle.get_location()).next_until_lane_end(20)) > 10:
                            park_state = park_vehicle(Maneuver_Agent, world)
                        else:
                            print("No parking spot found. Stopping vehicle...")
                            Maneuver_Agent.vehicle.apply_control(carla.VehicleControl(brake=1, hand_brake=True))
                            park_state = True
                    elif event.key == pygame.K_e:
                        print('Hospital')
                        move_to_hospital(agent, Maneuver_Agent.vehicle, hospitals_locations,Map)
