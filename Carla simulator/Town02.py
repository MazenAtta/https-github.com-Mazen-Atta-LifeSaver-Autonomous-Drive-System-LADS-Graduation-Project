...
def monitor_file():
    ...
    while True:
        ...
        try:
            file = open(r'D:\Projects\Project3 Software\Carla simulator\hparkState.txt', 'r')
...
class ManeuverAgent(BasicAgent):
    ...
    def getLeftLaneWaypoints(self, offset=VEHICLE_VEL, separation=0.3):
    ...
    def getRightLaneWaypoints(self, offset=VEHICLE_VEL, separation=0.3):
        current_waypoint = self.world.get_map().get_waypoint(self.vehicle.get_location())
...
def move_to_hospital(Maneuver_Agent, hospitals_locations, world):
    state = 2
    loc = 5
    ...
    while True:
        if state == 1 and loc > 20:
            near_location_ind,loc = get_nearest_hospital(Maneuver_Agent.vehicle, hospitals_locations, world.get_map())
            Maneuver_Agent.set_destination(hospitals_locations[near_location_ind].location)

        if state == 2:
            near_location,loc = get_nearest_hospital(Maneuver_Agent.vehicle, hospitals_locations, world.get_map())
            Maneuver_Agent.set_destination(hospitals_locations[near_location].location)
            state = 0
        
        # light_state, traffic_light = Maneuver_Agent._is_light_red(lights_list)
        # if light_state:
        #     traffic_light.set_state(carla.TrafficLightState.Green)            
        
        
        # for traffic_light in lights_list:
        #     if traffic_light.state == carla.TrafficLightState.Red or traffic_light.state == carla.TrafficLightState.Yellow:
        #         traffic_light.set_state(carla.TrafficLightState.Green)
        ...
        CarSpeed = Maneuver_Agent.vehicle.get_velocity().length()
        steer_angle = Maneuver_Agent.vehicle.get_control().steer
    ...
    while parking_mode == False:
        ...
        if not parking_mode:
            ...
            if ParkState == 1:
                print('Hospital')
                parking_mode == True
                move_to_hospital(Maneuver_Agent, hospitals_locations,world)
